        ...
        @endif
            ...
            @endif
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <input type="text" name="title" placeholder="Title" value="{{ $faculty->title }}"
                        class="border rounded px-3 py-2" required>
                    <input type="text" name="name" placeholder="Name" value="{{ $faculty->name }}"
                        class="border rounded px-3 py-2" required>
                    <input type="text" name="designation" placeholder="Designation" value="{{ $faculty->designation }}"
                        class="border rounded px-3 py-2" required {{ auth('teacher')->check() ? 'readonly' : ' ' }}>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <select name="department_id" class="border rounded px-3 py-2" required>
                        ...
                        @foreach($departments as $d)
                            <option value="{{ $d->id }}" @if($faculty->department_id == $d->id) selected @endif>{{ $d->name }}
                            </option>
                        @endforeach
                    </select>
                    <input type="email" name="email" placeholder="Email" value="{{ $faculty->email }}"
                        class="border rounded px-3 py-2" required>
                    <input type="text" name="mobile" placeholder="Mobile" value="{{ $faculty->mobile }}"
                        class="border rounded px-3 py-2" required>
                </div>
                    ...
                    <div id="facultyDetailsWrapper" class="space-y-2">
                        ...
                        @if($faculty->details->count() == 0)
                            ...
                            <select name="details[0][paper_master_id]" class="border rounded px-3 py-2" required>
                                ...
                                @foreach($papers as $p)
                                    <option value="{{ $p->id }}" @if($d->paper_master_id == $p->id) selected @endif>{{ $p->name }}
                                    </option>
                                @endforeach
                            </select>
                        @endif
                        @foreach($faculty->details as $index => $d)
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end mb-3">
                                <select name="details[{{ $index }}][course_id]" class="border rounded px-3 py-2" required>
                                    <option value="">Select Course</option>
                                    @foreach($courses as $c)
                                        <option value="{{ $c->id }}" @if($d->course_id == $c->id) selected @endif>{{ $c->name }}
                                        </option>
                                    @endforeach
                                </select>

                                {{-- Paper --}}
                                <select name="details[{{ $index }}][paper_master_id]"
                                    class="paper-select border rounded px-3 py-2" required>
                                    <option value="">Select Paper</option>
                                    @foreach($papers as $p)
                                        <option value="{{ $p->id }}" @if($d->paper_master_id == $p->id) selected @endif>{{ $p->name }}
                                        </option>
                                ...
                                <button type="button" onclick="this.parentElement.remove()"
                                    class="w-10 h-10 bg-red-600 text-white rounded flex items-center justify-center">-</button>
                            </div>
                        @endforeach
                    </div>

                    <button type="button" onclick="addFacultyDetail()"
