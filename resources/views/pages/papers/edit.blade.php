        ...
        <div class="bg-white shadow rounded-xl p-6">
            <form action="{{ route('paper.update', $paper->id) }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="paper_type"
                            ...
                            @foreach ($paperTypes as $type)
                                <option value="{{ $type }}" {{ $paper->paper_type == $type ? 'selected' : '' }}>{{ $type }}
                                </option>
                            @endforeach
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Eligibilty <span class="text-red-500"></span>
                        </label>
                        <input type="text" name="eligibilty" value="{{ $paper->eligibilty }}"
                            class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">
                            Prerequisites <span class="text-red-500"></span>
                        </label>
                        <input type="text" name="prerequisites" value="{{ $paper->prerequisites }}"
                            class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                            placeholder="Enter a Text">
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="capping" value="{{ $paper->capping }}"
                            class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="remark" value="{{ $paper->remark }}"
                            class="w-full rounded-lg border px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                            placeholder="Enter a Text">
                    </div>
                </div>
